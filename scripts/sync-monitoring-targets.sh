#!/usr/bin/env bash
# Wires up the one part of the per-cluster monitoring stack that can't be
# plain GitOps: blackbox-exporter needs this cluster's own root CA cert (so
# it can validate *.platform.local's TLS chain without -k/
...
# creds.sh -- see that script and CLAUDE.md's "Lessons Learned" for why
# this can't be Terraform/GitOps.
#
# Fully local to one cluster (Milestone 5 redesign -- this used to copy
# CAs across clusters into a separate management cluster; dropped along
# with that cluster, see CLAUDE.md's GitHub Actions Runners section): the
# CA cert just needs copying from cert-manager's namespace into monitoring's
# (Secrets don't cross namespaces on their own), and the Gateway IP is this
# same cluster's own.
#
...
if [ ! -f "$kubeconfig" ]; then
  echo "error: kubeconfig not found at ${kubeconfig}. Run 'make bootstrap' (or 'make bootstrap-prod') first." >&2
  exit 1
fi
